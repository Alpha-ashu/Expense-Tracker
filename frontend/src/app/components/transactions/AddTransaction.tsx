...
const PremiumModeSelector = ({ 
  ...
  return (
    <div className={cn(variant === 'pill' ? "mode-selector-pill" : "flex gap-1 bg-slate-100/50 p-1 rounded-xl", className)}>
      {variant === 'pill' && (
        <motion.div 
      ...
      {options.map(opt => (
        <button
          ...
          className={cn(
            ...
            variant === 'pill' 
              ? cn("mode-selector-btn", activeId === opt.id && "active")
              : activeId === opt.id ? "bg-white text-indigo-600 shadow-sm" : "text-slate-400 hover:bg-white/50"
          )}
        >
          {opt.icon}
          {opt.label}
      ...
      <main className="flex-1 p-3 lg:p-5 grid grid-cols-1 lg:grid-cols-12 gap-3 lg:gap-5 lg:overflow-hidden lg:overflow-y-auto pb-32 lg:pb-5">
        ...
        <div className="lg:col-span-5 flex flex-col gap-4 lg:overflow-y-auto">
          ...
          <div className="premium-glass-card p-6 bg-white relative overflow-hidden flex flex-col items-center">
             ...
             <div className="flex gap-1.5 mt-4">
                {[100, 500, 1000].map(amt => (
                  <button key={amt} onClick={() => { setAmountStr(String(amt)); setFormData(prev => ({ ...prev, amount: amt })); }} className="px-3 py-1.5 bg-slate-50 border border-slate-100 rounded-lg text-[10px] font-black text-slate-500 hover:bg-white hover:shadow-sm transition-all">+{amt}</button>
                ))}
             </div>
          </div>
          ...
          <div className="premium-glass-card p-4 space-y-3">
            <div className="space-y-1">
              <label className="text-[8px] font-black text-slate-400 uppercase tracking-widest">{isTransfer ? 'From Account' : 'Account'}</label>
              <SearchableDropdown
                ...
                placeholder="Account"
                className="h-10 rounded-xl border-none bg-slate-50 font-bold text-xs"
              />
            </div>
            ...
            {(isTransfer && transferSubType === 'self') && (
              <div className="space-y-3 animate-in fade-in zoom-in-95 duration-200">
                ...
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">To Account</label>
                <SearchableDropdown
                  ...
                  className="h-12 rounded-xl border-none bg-slate-50 font-bold text-sm"
                />
              </div>
            )}
            ...
            {isTransfer && transferSubType === 'others' && (
              <div className="space-y-3 animate-in fade-in zoom-in-95 duration-200">
                <div className="flex justify-center"><ArrowDown size={14} className="text-slate-300" /></div>
                <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Recipient Info</label>
                <div className="relative">
            ...
            <div className="grid grid-cols-2 gap-4">
               ...
               <div className="space-y-1.5">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Ref # (Optional)</label>
          ...
          {/* Intelligent Summary Footer */}
          <div className="mt-auto p-4 bg-slate-900 rounded-2xl text-white flex items-center justify-between shadow-xl">
             <div className="flex items-center gap-3">
