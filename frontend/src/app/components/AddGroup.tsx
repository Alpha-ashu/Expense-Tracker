...
const groupCategoryOptions = GROUP_CATEGORIES.map((category) => ({
  ...
  group: category.value === 'general' ? 'General' : 'Expense categories',
}));
            ...
            {friends.map((f) => {
              ...
              return (
                <button key={f.id} type="button" onClick={() => !added && addFriend(f.name)} disabled={added}
                  className={cn('rounded-lg px-3 py-1.5 text-xs font-semibold transition-all', added ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : ' border border-violet-300 text-violet-800 hover:bg-violet-100')}>
        ...
        {formData.participants.map((p, i) => (
          <div key={i} className="flex gap-2 items-center">
            ...
            <input type="text" value={p} onChange={(e) => updateParticipant(i, e.target.value)} placeholder={`Participant ${i + 1}`}
              className="flex-1 rounded-xl border border-gray-200 bg-gray-50 px-3 py-2 text-sm font-medium text-gray-900 focus:ring-2 focus:ring-violet-400 focus:border-violet-400 focus: outline-none" />
      ...
      {isDesktop ? (
        <div className="w-full min-h-[100dvh] bg-[radial-gradient(circle_at_top_left,#dbeafe_0%,#eef2ff_28%,#f8fafc_56%,#f8fafc_100%)] py-4 lg:py-7 font-sans">
          <div className="w-full max-w-[800px] mx-auto px-8 py-6">
            <div className="mb-6 flex items-center gap-3">
              <button type="button" onClick={() => setCurrentPage('groups')} className="flex h-9 w-9 items-center justify-center rounded-xl border border-gray-200  text-gray-500 hover:bg-gray-50 shadow-sm"><ChevronLeft size={18} /></button>
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-6">
                ...
                <div className="grid grid-cols-2 gap-4 items-end">
                  ...
                  <SearchableDropdown
                    ...
                    searchPlaceholder="Search group expense category..."
                    grouped
                  />
                  <div>
                  ...
                  <div className="col-span-2">
                    <button type="submit" disabled={isSubmitting}
                      className="h-12 w-full px-8 rounded-lg bg-gradient-to-r from-violet-600 to-purple-600 text-white font-bold shadow-lg hover:from-violet-700 hover:to-purple-700 transition-all active:scale-95 flex items-center justify-center gap-2 disabled:opacity-50">
        ...
        <div className="w-full min-h-[100dvh] bg-[radial-gradient(circle_at_top_left,#dbeafe_0%,#eef2ff_28%,#f8fafc_56%,#f8fafc_100%)] py-4 lg:py-7 font-sans flex flex-col">
          ...
          <div className="px-6 flex-1">
            <div className=" rounded-[32px] p-6 ring-1 ring-gray-100 shadow-[0_8px_40px_-12px_rgba(0,0,0,0.05)]">
              <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                  <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 ml-1">Group Name</label>
                  <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                    className="w-full bg-gray-50 border-0 rounded-2xl px-4 py-3.5 text-gray-900 font-medium text-lg placeholder-gray-400 focus:ring-2 focus:ring-violet-500 focus: transition-all" placeholder="e.g., Goa Trip" required />
                </div>
                
                <div>
                  <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 ml-1">Total Amount</label>
                  <div className="flex items-center bg-gray-50 rounded-2xl px-4 py-3.5 focus-within:ring-2 focus-within:ring-violet-500 focus-within: transition-all">
                ...
                <div>
                  ...
                  <input type="date" value={formData.date} onChange={(e) => setFormData({ ...formData, date: e.target.value })}
                    className="w-full bg-gray-50 border-0 rounded-2xl px-4 py-3.5 text-gray-900 font-medium focus:ring-2 focus:ring-violet-500 focus: transition-all" />
                </div>
